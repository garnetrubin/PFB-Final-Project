...
// 1. GET USER'S AGE (To calculate needs)
$age_query = $conn->query("SELECT age FROM users WHERE id = $user_id");
$user_data = $age_query->fetch_assoc();
$user_age = $user_data['age'];

// Define Water Goal based on Age
$water_goal = ($user_age < 18) ? 2.0 : 2.5; // Teens need ~2L, Adults ~2.5L

// --- SECTION 1: SAVE NEW DATA ---
...
$sleep_data = [];
$logs = []; 
$latest_log = null; // We will store the most recent log here to analyze it

while($row = $result->fetch_assoc()) {
    $dates[] = $row['log_date'];
    $sleep_data[] = $row['sleep_hours'];
    $logs[] = $row;
    $latest_log = $row; // The loop ends with the latest data
}
...
<!-- --- NEW FEATURE: SMART HEALTH ADVICE --- -->
<?php if ($latest_log): ?>
...
<div style="overflow-x: auto;">
    <table>
        ...
        <?php if (count($logs) > 0): ?>
            <?php foreach(array_reverse($logs) as $row): ?> <!-- Reversed to show newest first -->
            <tr>
